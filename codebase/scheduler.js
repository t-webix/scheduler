...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var EventFormView = (function (_super) {
        ...
        EventFormView.prototype.UpdateEvent = function (change) {
            ...
            if (sid === "0") {
                ...
                if ((mode === "this" && !this.HasOneOccurrence(obj)) || mode === "next") {
                    ...
                    this._inProgress = this.Ops.updateEvent(sid, { recurring: recurring }, mode, webix.Date.dayStart(newEvent_1.start_date), true).then(function () {
                    ...
                    this._inProgress = this.Ops.updateEvent(this.Id, change, mode, undefined, true).then(function () {
                        if (change.rec_option === "none")
                            _this.State.selected.date = null;
                    });
                }
        ...
        EventFormView.prototype.UpdateSelected = function (id) {
        ...
        EventFormView.prototype.PrepareChange = function (change, prev) {
    ...
    var units = webix.extend({
        ...
        $drop: function (source, target, e) {
            ...
            var date = this.getActualDate(this.master);
            var units = this.getEventUnits(ctx.event, ctx.from);
            if (date || units) {
                ...
                if (ctx.event.id == "0") {
                    ...
                    this.updateEvent(ctx.event, update, ctx);
            }
        },
        getEventUnits: function (event, source) {
            var currentUnits = event.units.split(",");
            var targetUnit = this.master.Unit.id + "";
            var sourceUnit = source.master.Unit.id + "";
    ...
    var timeline = webix.extend({
        ...
        locate: function (e) {
            ...
            var y = webix.html.pos(e).y - ctx.offset.y + nscroll.y;
            var height = this.view.$scope.Sections.count();
            var res = Math.min(Math.floor(y / this.view.type.height), height - 1);
            return res;
        },
        $dragCreate: function (target, e) {
            var ctx = this.getContext();
        ...
        $dragIn: function (source, target, e) {
            ...
            if (ctx.from.mode !== this.mode)
                return false;
            ctx.offset = ctx.offset || webix.html.offset(target);
            var id = this.view.getIdByIndex(this.locate(e));
            var node = this.view.getItemNode(id);
            if (ctx.moveMode === "move" &&
        ...
        $dragPos: function (pos) {
            var ctx = this.getContext();
            var x = ctx.x, t = ctx.t, l = ctx.l, w = ctx.w, offset = ctx.offset, scroll = ctx.scroll, step = ctx.step, moveMode = ctx.moveMode;
            var nscroll = this.view.getScrollState();
        ...
        $drop: function (source, target, e) {
            ...
            if (data) {
                if (ctx.event.id == "0") {
                    ...
                    this.updateEvent(ctx.event, data, ctx);
            }
        },
        getData: function (e) {
            var ctx = this.getContext();
    ...
    var timelineMore = webix.extend({
        ...
        $dragCreate: function (target, e) {
            ...
            if (id) {
                ...
                var node = this.view.getItemNode(id).children[0];
                webix.extend(ctx, { from: this, id: id, moveMode: "move" }, true);
                ctx.event = this.master.Events.getItem(id);
    ...
    function switchCursor(e, view) {
        if (webix.DragControl.active)
            return;
        var _a = locateEvent(e, view), id = _a.id, node = _a.node;
        if (id) {
    ...
    var ChartView = (function (_super) {
        ...
        ChartView.prototype.config = function () {
            ...
            var active = webix.skin.$active;
            this._sectionSize = 5;
            var height = (active.barHeight - active.borderWidth * 2) * this._sectionSize;
            return {
                cols: [
                    {
                        view: "treetable",
        ...
        ChartView.prototype.init = function () {
            ...
            if (!webix.env.touch)
                this._mousemove_handler = webix.event(this.Bars.$view, "mousemove", function (e) {
                    return switchCursor(e, _this.Bars.$view);
                });
        };
        ...
        ChartView.prototype.RefreshTask = function (updID) {
            var _this = this;
            var t = this.Events.getItem(updID);
            ...
            var prev = this._processed.find(function (obj) {
                if (obj.section == t.section) {
                    ...
                    var prev_1 = webix.Date.timePart(obj.end_date)
                        ? wgantt_3(unit, webix.Date.add(obj.end_date, 1, unit, true))
                        : obj.end_date;
                    var next = wgantt_3(unit, t.start_date);
            ...
            wgantt_9(t, this._lastRow[t.section] + sectionIndex * this._sectionSize, this.Scales, this.Scales.cellHeight - 12, y);
        ...
        ChartView.prototype.DrawGrid = function (scales) {
            var view = this.Bars;
            view.$view.style.backgroundImage = "url(" + wgantt_4(scales.cellWidth, scales.cellHeight, webix.skin.$name == "contrast" ? "#808080" : "#ddd") + ")";
            ...
            if (obj.all_day && obj.end_date < scales.end) {
                w +=
                    scales.precise && scales.minUnit === "week"
                        ? Math.floor(scales.cellWidth / wgantt_7[scales.minUnit][1])
                        : scales.cellWidth;
            }
        ...
        ChartView.prototype.ShowEvent = function (e) {
            var event = locateEvent(e, this.Bars.$view);
        ...
        ChartView.prototype.ShowNew = function (e, id) {
            if (!this.State.readonly) {
                ...
                var unit = scales.precise
                    ? wgantt_7[scales.minUnit][0]
                    : scales.minUnit;
                var date = wgantt_1(unit)(start, n);
                this.State.selected = { id: "0", date: date, section: id };
            }
        };
        return ChartView;
    }(JetView));
    ...
    var ScalesView = (function (_super) {
        ...
        ScalesView.prototype.RenderScales = function () {
            ...
            var html = scales.rows
                ...
                return ("<div class=\"webix_scheduler_timeline_scale_row\" style='height:" + line.height + "px;width:" + scales.width + "px'>" +
                    line.cells.map(function (cell) { return _this.CellTemplate(cell); }).join("") +
                    "</div>");
            })
                .join("");
        ...
        ScalesView.prototype.CellTemplate = function (cell) {
            return "<div class=\"webix_scheduler_timeline_scale_cell " + cell.css + "\" style=\"width:" + cell.width + "px;\">" + cell.format(cell.date) + "</div>";
        };
        return ScalesView;
    }(JetView));
    ...
    var TimelineView = (function (_super) {
        ...
        TimelineView.prototype.SetScales = function (date, type) {
            var start = wgantt_3(type, date);
            var end = wgantt_1(type)(start, 1);
            var active = webix.skin.$active;
            var scales = wgantt_6(start, end, type === "month", this.GetScalesCellWidth(type), active.barHeight - active.borderWidth * 2, this.GetScalesArray(type));
    ...
    var UnitEventsView = (function (_super) {
        ...
        UnitEventsView.prototype.ShowNew = function (h) {
            if (!this.State.readonly) {
                ...
                date.setHours(h);
                this.State.selected = { id: "0", date: date, unit: unit };
            }
        };
    ...
    var UnitMultidayView = (function (_super) {
        ...
        UnitMultidayView.prototype.init = function () {
            ...
            this.on(this.app, "units:multi:state", function (state, animate) {
                var delay = animate ? 150 : 1;
                _this.Animate(state, delay);
            });
        };
